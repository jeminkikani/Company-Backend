...
module.exports.validationConstant = function (method) {
    ...
    const checkMissingFields = (req, res, next) => {
        ...
        if (missingFields.length > 0 || extraFields.length > 0) {
            ...
            if (missingFields.length > 0) {
                errorFields.push(`Missing required fields: ${missingFields.join(', ')}`);
            }
            if (extraFields.length > 0) {
                errorFields.push(`Extra fields is not Valid: ${extraFields.join(', ')}`);
            }
    ...
    const validateUpdate = (req, res, next) => {
        ...
        const newFields = Object.keys(req.body).filter(field => !validateFields.includes(field));
    ...
    switch (method) {
        case "register":
            ...
            return [validator.FIRSTNAME, validator.LASTNAME, validator.EMAIL, validator.PASSWORD, validator.ROLE, checkMissingFields];
        case "login":
            return [validator.EMAIL, validator.PASSWORD];
        case "update":
            return [validator.FIRSTNAME, validator.LASTNAME, validator.ROLE, validateUpdate];
    }
};
